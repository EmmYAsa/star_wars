    ...
    links.forEach(link => {
        link.parentElement.addEventListener('click', () => {
            ...
            if (!isNaN(linkContentParseInt)) {
                getVehicle(linkContentParseInt);
                ...
                if (linkContent == '«' && !arrowLeft.classList.contains('disabled') && parseInt(currentActiveLink.textContent) > 1) {
                    ...
                    if (previousLink) {
                        ...
                        getVehicle(parseInt(previousLink.textContent));
                ...
                if (linkContent == '»' && !arrowRight.classList.contains('disabled') && parseInt(currentActiveLink.textContent) < 4) {
                    ...
                    if (nextLink) {
                        ...
                        getVehicle(parseInt(nextLink.textContent));
...
function showAllVehicles(data) {
    ...
    data.forEach(element => {
        const vehicleId = element.url.match(/\/([0-9]*)\/$/)[1];
...
function showDetails(data, url){
    ...
    const {name, cargo_capacity, consumables, length, cost_in_credits} = data

    li[0].textContent = name
    li[1].textContent = cargo_capacity
    li[2].textContent = length
